...
import { useDroppable, DragItem } from '../hooks/useDragAndDrop'
...
interface FolderTreeProps {
  ...
  onFileDrop?: (item: DragItem, targetFolderId?: string) => void
...
const FolderNode: React.FC<FolderNodeProps> = ({
  ...
  // Make folder droppable for file drops
  const dropProps = useDroppable(
    `folder-${folder.id}`,
    ['file'],
    (item) => onFileDrop?.(item, folder.id)
  )
  ...
  return (
    <div>
      <div
        {...dropProps}
        className={`flex items-center group hover:bg-gray-50 rounded-lg p-2 cursor-pointer ${
...
export const FolderTree: React.FC<FolderTreeProps> = ({
  ...
  // Root folder drop props
  const rootDropProps = useDroppable(
    'root-folder',
    ['file'],
    (item) => onFileDrop?.(item, undefined)
  )
